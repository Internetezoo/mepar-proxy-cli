...
// ✅ KRITIKUS JAVÍTÁS: HOZZÁADVA A +towgs84=52.17,-71.82,-14.9,0.0,0.0,0.0,0.0 PARAMÉTERT az EOV definícióhoz
proj4.defs("EPSG:23700", "+proj=somerc +lat_0=47.14439372222222 +lon_0=19.04857177777778 +k=0.99993 +x_0=650000 +y_0=200000 +ellps=GRS67 +units=m +no_defs +towgs84=52.17,-71.82,-14.9,0.0,0.0,0.0,0.0");

// KRITIKUS: A FÖMI NTA WMS CÉL-URL-JE
const NTA_WMS_URL = 'http://tkp.fomi.hu/mapservice/nta/lf/teir/wms';

const TARGET_CRS = 'EPSG:23700'; 
const NTA_LAYER = 'ntalf'; 
const TILE_SIZE = 256;
...
module.exports = async (req, res) => {
    ...
    try {
        const { z, y, x, FORMAT = 'image/png' } = req.query; 
        ...
        // WMS 1.1.0 verzióhoz a [minX, minY, maxX, maxY] standard sorrend
        const final_bbox = `${eov_minX},${eov_minY},${eov_maxX},${eov_maxY}`;
        
        // 4. FÁZIS: WMS URL felépítése (WMS 1.1.0 verzióval, mint az Ön által felfedezett linkekben)
        const targetUrl = `${NTA_WMS_URL}?service=WMS&request=GetMap&version=1.1.0&layers=${NTA_LAYER}&styles=&srs=${TARGET_CRS}&bbox=${final_bbox}&width=${TILE_SIZE}&height=${TILE_SIZE}&format=${FORMAT}&transparent=true`;
