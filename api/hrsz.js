...
// ✅ KRITIKUS JAVÍTÁS: PONTOSAN UGYANAZT A +towgs84 PARAMÉTERT HASZNÁLJUK, MINT AZ NTA.JS-BEN!
...
function calculateBboxFromTile(matrixId, tileCol, tileRow) {
    ...
    // A BBOX helyes sorrendben [minX, minY, maxX, maxY]
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (isNaN(zInt) || isNaN(yInt) || isNaN(xInt) || zInt < 0 || yInt < 0 || xInt < 0 || !LAYER) {
            return res.status(400).send('Érvénytelen vagy hiányzó paraméterek (z, x, y, LAYER).');
        }
        ...
        // 4. FÁZIS: WMS URL felépítése (VERSION=1.1.0 és SRS)
